...
%>
    ...
    <form name=ChangeForm method=post action="/ManagementPageChangefunction.jsp?eaterynumber=${eaterynumber}" enctype="multipart/form-data">
   	...
   	%>
      	 <tr>
            <td>상호명 : </td>
            <td colspan="2"><input type="text" name="eateryNameBack" value="<%= eateryfumigatorList.get(i).getEateryName() %>"></td>
         </tr>
         <tr>
         <td>업종 : </td>
			<td colspan="2"><select id="eaterycategory" name ="eaterycategory">
         ...
         <tr>
            <td> 비대면 주문 : </td>
            <td colspan="2"><input type="radio" name="untactBack" value ="O" <%=("O".equals(eateryfumigatorList.get(i).getUntact()))?"checked":""%>>O  &nbsp;&nbsp;&nbsp;
            	<input type="radio" name="untactBack" value ="X" <%=("X".equals(eateryfumigatorList.get(i).getUntact()))?"checked":""%>>X
            </td>
         </tr>
         <tr>
            <td> 포장 : </td>
            <td colspan="2"><input type="radio" name="packingBack" value ="O" <%=("O".equals(eateryfumigatorList.get(i).getPacking()))?"checked":""%>>O &nbsp;&nbsp;&nbsp;
            	<input type="radio" name="packingBack" value ="X" <%=("X".equals(eateryfumigatorList.get(i).getPacking()))?"checked":""%>>X
            </td>
         </tr>
         <tr>
            <td> 배달 : </td>
            <td colspan="2"><input type="radio" name="deliverBack" value ="O" <%=("O".equals(eateryfumigatorList.get(i).getDeliver()))?"checked":""%>>O &nbsp;&nbsp;&nbsp;
         ...
         <tr>
            ...
            %>
               ...
               <input id = "page" type="button" value="파일 삭제" onClick ="pageDelete(<%=eateryfumigatorList.get(i).getEaterynumber()%>,<%=fileList.get(j).getFile_idx()%>)"/><br>
            
            <%      
               }
            %>
            <table class="table table-hover" style="text-align: center; " id="dynamic_table">
            <input type="hidden" name="count" ></table></td>
            <td><input class="btn btn-secondary" type="button" value="추가" onclick="javascript:addInputBox();">  <input class="btn btn-secondary" type="button" value="삭제" onclick="javascript:subtractInputBox();"><br><br></td>
      ...
      <div class="text-center">
	      ...
	      <input class="btn btn-secondary" type="button" value="취소" onclick="location.href='/ManagementPage.jsp?eaterynumber=${eaterynumber}' " >
      </div>
      </form>
